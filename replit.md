...
1. **Family Galaxy** - 3D WebGL-powered family constellation inspired by No Man's Sky galaxy map
   ...
   - **Connection Lines**: Mesh-based thick quad lines (4px screen-space width) with GLSL shader-based energy pulse animation; household-colored; lines terminate at GalaxyOutlineRing perimeter (offset along direction to other household) for coupled households, center for non-coupled; nearly invisible at rest (hlVal 0.0 → alpha 0.015), full brightness on hover (hlVal 1.0 → alpha 0.9); intra-household edges filtered out; cached _lineColor for per-frame reuse; ID type coercion for robust Map lookups
